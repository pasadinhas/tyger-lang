...
public class FunctionFormalParameter extends Typed {
    public String name;

    public FunctionFormalParameter(final TygerParser.FunctionFormalParameterContext ctx) {
        this.name = ctx.name.getText();
        this.type = ctx.expressionTypeHint() == null ? null : new Type(ctx.expressionTypeHint().primitiveType().getText());
    }
